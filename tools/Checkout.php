...
trait Checkout {
	...
	public function setEnvironment($sandbox = true) {
	...
	public function getSessionId() {
		...
		if ($httpConnection -> getStatus() === 200) {
			...
			throw new \Exception("API Request Error: " . $httpConnection -> getStatus() . " - " . $httpConnection -> getResponse());
	...
	public function doPayment($params) {
		...
		// get last sales orderm from logued customer
		...
		$params['currency'] = 'BRL';
		// Currency (only BRL)
		$params['reference'] = rand(0, 9999);
		// Setting the Application Order to Reference on PagSeguro
